...
var View = {
    ...
    generateGrid: function(callback) {
        ...
        createRowTask = function(rowId) {
            return function(done) {
                ...
                var percent = Math.round((rowId + 1) / numRows * 100);
                Controller.showLoadingStatus('generating grid ' + percent + '%');
                done(null);
            };
        };
    ...
    showStats: function(opts) {
        ...
        // Check if loading panel is visible
        if ($('#loading_panel').is(':visible')) {
            $stats.addClass('with-loading');
        } else {
            $stats.removeClass('with-loading');
        }
        
        $stats.html(texts.join('<br>')).fadeIn(200);
    },
